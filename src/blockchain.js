...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
           ...
           if(currentTime - messageTime < 5 * 60) {
               ...
               if(bitcoinMessage.verify(message, address, signature)){
                   ...
                   reject("message could not be verified with given wallet address " + address + " and signature " + signature);
               }
           } else {
               reject("Can not submit a star that was created more than 5 minutes ago");
           }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           let blockWithGivenHash = self.chain.filter(block => block.hash === hash)[0];
           if(blockWithGivenHash) {
               resolve(blockWithGivenHash);
           } else {
               reject("Could not find a block with hash " + hash);
           }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async (block) => {
                ...
                if(!validBlock || block.previousBlockHash != previousBlockHash) {
                    errorLog.push(block);
                }
